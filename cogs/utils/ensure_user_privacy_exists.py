...
class EnsureUserPrivacyExists(commands.Cog):
  def __init__(self, bot):
    self.bot:commands.Bot = bot
  
  async def ensure_user_privacy_exists(self,user_id:int,guild:nextcord.Guild=None):
    try:
      try:
      ...
      log = nextcord.Embed(
        title=f"Postgresql | Ошибка В Привязке Пользователя К Флагам",
        description=(f"{e}")[:500],
...
def setup(bot:commands.Bot):
  bot.add_cog(EnsureUserPrivacyExists(bot))