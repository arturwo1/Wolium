...
class reportmodal(Modal):
  def __init__(self, interaction:Interaction, message:Message, language:str, mod_channel_id: int, bot:commands.Bot,timeout=60*5):
    super().__init__(title=translate_to_all_languages("Жалоба На Сообщение", 'message', language),timeout=timeout)
  ...
  async def callback(self, interaction: Interaction):
    ...
    await interaction.followup.send(await (TranslateMessage(self.bot)).translate_message(f"Вы Успешно Пожаловались На Сообщение Пользователя! Ваша Жалоба Была Отправлена.",self.language),ephemeral=True)
    ...
    fields = [
      ...
      {
        'name':await (TranslateMessage(self.bot)).translate_message('Сообщение',self.language),
        'value':str(self.message.content),
        'inline':True
      },
      {
    ...
    await (SendEmbed(self.bot)).send_embed(
      title="Репорт",
      description=f"### **{interaction.user.mention}** Репортнул Сообщение **{self.message.author.mention}**, С Содержанием:\n-# {self.message.content}\n### По Причине: **`{self.reason.value}`**",
      color=Colour.yellow(),
      fields=fields,
      footer_text='Жалоба На Сообщение',
    ...
    messagemod, modembed = await (SendEmbed(self.bot)).send_embed(
      ...
      description=f"### **{interaction.user.mention}** {await (TranslateMessage(self.bot)).translate_message('Репортнул Сообщение',self.language)} **{self.message.author.mention}**, {await (TranslateMessage(self.bot)).translate_message('С Содержанием:',self.language)}\n-# {self.message.content}\n### {await (TranslateMessage(self.bot)).translate_message('По Причине:',self.language)} **`{self.reason.value}`**",
      color=Colour.yellow(),
      fields=fields[1:],
      footer_text=await (TranslateMessage(self.bot)).translate_message('Жалоба На Сообщение',self.language),
      author_text=interaction.user.name,
      author_icon=interaction.user.display_avatar.url,
...
class MessageReport(commands.Cog):
  ...
  name_localizations=translate_to_all_languages('Репорт Сообщения', 'description'))
  ...
  setattr(репорт_сообщения,"extras",{"description": "С Помощью Этой Команды Вы Можете Репортнуть Сообщение Пользователя!"})
