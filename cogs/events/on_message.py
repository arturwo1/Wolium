...
class OnMessage(commands.Cog):
    ...
    try:
      if message.author!=self.bot.user and (any(trigger in message.content.lower() for trigger in bot_triggers) or (self.bot.user in message.mentions) or ((((message.reference.resolved if message.reference.resolved else (await message.channel.fetch_message(message.reference.message_id))).author.id if message.reference.message_id else 0) if message.reference else 0) == self.bot.user.id)) and message.author.id!=self.bot.user.id and len(message.content.strip()) > 1 and (message.guild.me.guild_permissions.send_messages if message.guild else True):
        try:
          ...
          if message.guild and get_data:
            guild_config = await get_data.get_data(message.guild.id,['aibot'],'guild_settings','guild_id',message.guild)
          ...
          async with message.channel.typing():
            ...
            if AI_message not in [None,'None','']:
              try:
                ...
                if not "Не Удалось Сгенерировать Сообщение." in str(AI_message) and send_embed:
                  fields = [
                    {
                      'name':'Пользователь',
              ...
              try:
                await message.reply(await translate_message.translate_message('М-м, не знаю, что сказать 🤔',language), delete_after=7)
              except Exception:
                pass
  ...
  async def games(self, message: Message, language: str):
    ...
    guild_config = await get_data.get_data(message.guild.id,['word_channel', 'number_channel', 'words', 'filter'],'guild_settings','guild_id',message.guild)
    ...
    if word_channel and message.channel.id == word_channel:
      if last_message and last_message.author.id == self.bot.user.id and message.author.id != self.bot.user.id:
        await message.reply(await translate_message.translate_message('Подождите Пока Я Удалю Сообщения Лишние И Потом Уже Пишите!', language), delete_after=3)
        await message.delete(delay=1)
        return

      if content in words:
        await message.reply(await translate_message.translate_message('Это Слово Уже Было!', language), delete_after=7)
        await message.delete(delay=7)
        return

      if last_message and message.author.id == last_message.author.id:
        await message.reply(await translate_message.translate_message('Не Ваша Очередь!', language), delete_after=7)
        await message.delete(delay=7)
        return
      ...
      if last_message:
        ...
        if first_current_letter != last_last_letter:
          await message.reply(await translate_message.translate_message('Ваше Слово Должно Начинаться На', language) + f' **`{last_last_letter}`**!\n' +await translate_message.translate_message('Ваше Слово Начинается На', language) + f' **`{first_current_letter}`**!',delete_after=7)
          await message.delete(delay=7)
          return
      ...
      if filter=='extreme':
        ...
        if last_message.reactions:
          ...
          for reaction in reactions:
            ...
            if reaction.emoji!='✅':
              ...
              checked=True
        if not checked:
          await message.reply(await translate_message.translate_message('Я Еще Не Проверил Сообщение! Подождите Пока Я Поставлю Галочку.', language), delete_after=15)
          await message.delete(delay=7)
          return
        ...
        try:
          ...
          await message.reply(await translate_message.translate_message('Произошла Ошибка При Определении Языка Вашего Сообщения:', language)+f" **```py\n{e}```**\n"+await translate_message.translate_message('Если Вы Не Знаете, Что Делать, Пришлите Скриншот Этой Ошибке На Оффициальном Моем Сервере В Канале Нужном!\nВозможно Сообщение Слишком Короткое.', language), delete_after=15)
          await message.delete(delay=7)
          return
        lang_code = "ru" if lang_code in ['bg','uk','mk'] else lang_code
        if lang_code not in supp_langs:
          await message.reply(await translate_message.translate_message('Язык Вашего Сообщения Не Добавлен В Базу',language)+f'(`{lang_code}`)!\n'+await translate_message.translate_message('Выключите Сильный Фильтр Слов в `/настройки`.\n-# когда вы все настроите, удалите это сообщение.\n-# Поддерживаемые языки:', language)+f" **`{'; '.join(f'{supp_lang}' for supp_lang in supp_langs)}`**",delete_after=15)
          await message.delete(delay=7)
          return
        try:
          spell = await self.get_spellchecker_cached(lang_code)
        except ValueError as e:
          await message.reply(await translate_message.translate_message('Словарь для этого языка не установлен. Пожалуйста, выберите другой язык.', language)+f"\n**```py\n{e}```**\n", delete_after=15)
          await message.delete(delay=7)
          return
        if content not in spell:
          ...
          if suggs:
            ...
            await message.reply(await translate_message.translate_message('Ваше Слово Содержит Ошибку!\nВозможно Вы Имели Ввиду:', language)+f" `{sugg}`", delete_after=7)
            await message.delete(delay=7)
            return
          await message.reply(await translate_message.translate_message('Недействительное Слово!', language), delete_after=7)
          await message.delete(delay=7)
          return
    ...
    if number_channel and message.channel.id == number_channel:
      ...
      if not current_number_match:
        await message.reply(await translate_message.translate_message('Вы Ввели Не Число!', language), delete_after=7)
        await message.delete(delay=7)
        return

      if last_message and not last_number_match:
        await message.reply(await translate_message.translate_message('В Прошлом Сообщении Не Число, Я Его Щас Удалю.', language), delete_after=7)
        await message.delete(delay=7)
      ...
      if current_number != last_number + 1:
        await message.reply(await translate_message.translate_message('Ваше Число Не Подходит!', language), delete_after=7)
        await message.delete(delay=7)
        return
  ...
  async def on_message(self, message: Message):
    ...
    if message.guild:
      guild_settings = await get_data.get_data(message.guild.id,['banned'],'guilds','guild_id',message.guild)
    user_settings = await get_data.get_data(user_id,['language','variation','banned'],'users','user_id',message.guild)
    ...
    invite = await get_invite.invite(message.guild)
    ...
    if message.guild:
      guild_config = await get_data.get_data(message.guild.id,['mod_log_channel','moderation','moderation_type','rules', 'ttl_channel'],'guild_settings','guild_id',message.guild)
    ...
    if message.author!=self.bot.user and message.guild:
      user_privacy = await get_data.get_data(user_id,['save_messages', 'save_message_data'], 'user_privacy', 'user_id', message.guild)
      ...
      user_data = await get_data.get_data(user_id,['xp','bank_balance','balance','upgrade'],'user_data','user_id',message.guild)
      xp = user_data['xp']
